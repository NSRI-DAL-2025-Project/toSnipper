...
#' @param input (character) File path to the xlsx, csv, or vcf file. If an xlsx or csv file, the first column should be "Sample" while the succeeding columns are genotypes with marker names as headers.
#' @param references (character) File path to the xlsx or csv file. It should contain three columns 1) Sample, 2) Population, and 3) Superpopulation/Continental region
#' @param target.pop (logical) Set TRUE if there is a target population to classify. Default is TRUE. Set to FALSE if all data will be used as reference.
#' @param population.name (character) Population name to be classified in SNIPPER.
#' @param markers (integer) The number of markers in the dataset.
#' @import pacman
#' @import tools
#' @import readr
#' @import readxl
